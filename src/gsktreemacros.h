...
#define GSK_RBTREE_LOOKUP_COMPARATOR(tree, key, key_comparator, out) \
  GSK_RBTREE_LOOKUP_COMPARATOR_(tree, out)
        ...
        while (top != _gsk_at && is_red(_gsk_at->parent))                      \
          {                                                                    \
            if (_gsk_at->parent == _gsk_at->parent->parent->left_child)        \
              ...
              {                                                                \
                type _gsk_y = _gsk_x->parent->parent->left;                    \
                if (is_red (_gsk_y))                                           \
                  {                                                            \
                    set_is_red (_gsk_at->parent, 0);                           \
                    set_is_red (_gsk_y, 0);                                    \
...
G_STMT_START{                                                                 \
  ...
  type _gsk_rb_del_x;                                                         \
  type _gsk_rb_del_y;                                                         \
  type _gsk_rb_del_nulpar = NULL;	/* Used to emulate sentinel nodes */  \
  gboolean _gsk_rb_del_fixup;                                                 \
  if (_gsk_rb_del_z->left_child == NULL || _gsk_rb_del_z->right_child == NULL)\
    _gsk_rb_del_y = _gsk_rb_del_z;                                            \
  else                                                                        \
    _gsk_rb_del_y = gsk_tree_node_next_internal (tree, _gsk_rb_del_z);        \
  ...
  if (_gsk_rb_del_x)                                                          \
    ...
    _gsk_rb_del_nulpar = _gsk_rb_del_y->parent;                               \
  if (!_gsk_rb_del_y->parent)                                                 \
    tree->top = _gsk_rb_del_x;                                                \
  else                                                                        \
    {                                                                         \
  ...
  if (_gsk_rb_del_y != _gsk_rb_del_z)                                         \
	...
	_gsk_rb_del_y->right_child->parent = _gsk_rb_del_y;                   \
      if (_gsk_rb_del_nulpar == _gsk_rb_del_z)                                \
	_gsk_rb_del_nulpar = _gsk_rb_del_y;                                   \
    }                                                                         \
  if (_gsk_rb_del_fixup)                                                      \
    {                                                                         \
      /* delete fixup (Algorithms, p 274) */                                  \
      while (_gsk_rb_del_x != tree->top && !is_red (_gsk_rb_del_x))           \
        {                                                                     \
          ...
          if (_gsk_rb_del_x == _gsk_rb_del_xparent->left_child)               \
            {                                                                 \
              ...
              if (is_red (w))                                                 \
                {                                                             \
                  set_is_red (w, 0);                                          \
                  set_is_red (xparent, 1);                                    \
                  gsk_tree_left_rot (tree, _gsk_rb_del_xparent);              \
                  w = xparent->right_child;                                   \
                }                                                             \
              if (!is_red (w->left_child)                                     \
               && !is_red (_gsk_rb_del_w->right_child))                       \
                {                                                             \
                ...
                {                                                             \
                  if (!is_red (_gsk_rb_del_w->right_child))                   \
                    {                                                         \
                      if (_gsk_rb_del_w->left_child)                          \
                        set_is_red (_gsk_rb_del_w->left_child, 0);            \
                      set_is_red (_gsk_rb_del_w, 1);                          \
                      gsk_tree_right_rot (tree, _gsk_rb_del_w);               \
            ...
            {                                                                 \
              ...
              if (!is_red (_gsk_rb_del_w->right_child)                        \
               && !is_red (_gsk_rb_del_w->left_child))                        \
                ...
                {                                                             \
                  if (!is_red (_gsk_rb_del_w->left))                          \
                    {                                                         \
                      set_is_red (_gsk_rb_del_w->right, 0);                   \
                      set_is_red (_gsk_rb_del_w, 1);                          \
                      gsk_tree_left_rot (tree, _gsk_rb_del_w);                \
                      _gsk_rb_del_w = _gsk_rb_del_xparent->left_child;        \
                    }                                                         \
                  set_is_red (_gsk_rb_del_w, is_red (xparent));               \
                  set_is_red (_gsk_rb_del_xparent, 0);                        \
                  if (_gsk_rb_del_w->left)                                    \
                    set_is_red (_gsk_rb_del_w->left, 0);                      \
                  gsk_tree_right_rot (tree, _gsk_rb_del_xparent);             \
                  _gsk_rb_del_x = tree->top;                                  \
                }                                                             \
            }                                                                 \
        }                                                                     \
  ...
  _gsk_rb_del_z->left_child = z->right_child = z->parent = NULL;              \
}G_STMT_END
...
G_STMT_START{                                                              \
  type _gsk_rot_y = (node)->right;                                         \
                                                                           \
  (node)->right = _gsk_rot_y->left;                                        \
  if (_gsk_rot_y->left)                                                    \
    _gsk_rot_y->left->parent = (node);                                     \
  _gsk_rot_y->parent = (node)->parent;                                     \
  if ((node)->parent == NULL)                                              \
    top = _gsk_rot_y;                                                      \
  else if ((node) == (node)->parent->left)                                 \
    (node)->parent->left = _gsk_rot_y;                                     \
  else                                                                     \
    (node)->parent->right = _gsk_rot_y;                                    \
  _gsk_rot_y->left = (node);                                               \
  (node)->parent = _gsk_rot_y;                                             \
}G_STMT_END
